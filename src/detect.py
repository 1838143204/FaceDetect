...
net1 = network.load_network_1('../model/net1_weights.h5')
net2 = network.load_network_2('../model/net2_weights.h5')
net3 = network.load_network_3('../model/net3_weights.h5')

graph = tf.get_default_graph()


def get_bounding_box(img, min_size=network.window_size, max_size=None, scale_factor=1.25, thresholds=[0.4, 0, 1],
    ...
    while scale >= min_scale:
        ...
        if new_rows >= network.window_size and new_cols >= network.window_size:
            ...
            bb, score = util_func.heatmap_to_bounding_box(feature_map, network.window_size, network.step_size,
                                                          thresholds[0])
